...
class SpectrogramCanvas(FigureCanvas):
	def __init__(self, window):
		...
		self.fftSize = 1024 # Initial FFT size just to render something in the charts
		self.sampleRate = 0
		self.binFreq = 0
	...
	def updateParameters(self, fftSize, sampleRate):
		...
		labels = ['%d hz' % i for i in np.linspace(0, self.sampleRate/2.0, 5)] #parece que este da el rango de la frecuencia
...
class MainWindow(QtGui.QMainWindow):
	...
	def _setupMainLayout(self):
		...
		author = QtGui.QLabel('Thanks to <a href="http://www.github.com/tdicola/">Tony DiCola</a>')
